...
bmap(struct inode *ip, uint bn)
{
  ...
  //start loading 2 Single indirect pointers
  if(bn < 2*NINDIRECT){

    uint indx = bn / NINDIRECT;
    if((addr = ip->addrs[NDIRECT + indx]) == 0)
      ip->addrs[NDIRECT + indx] = addr = balloc(ip->dev);
  ...
  if(bn < 1*NINDIRECT*NINDIRECT){

    uint idx = bn / (NINDIRECT*NINDIRECT); 
    if((addr = ip->addrs[NDIRECT + NINDIRECT + idx]) == 0)
      ip->addrs[NDIRECT + NINDIRECT + idx] = addr = balloc(ip->dev);
    bp = bread(ip->dev, addr);
    a = (uint*)bp->data;
    uint bn1 = bn % (NINDIRECT*NINDIRECT);

    
    if((addr = a[bn1/NINDIRECT]) == 0)
      a[bn1/NINDIRECT] = addr = balloc(ip->dev);
    bp1 = bread(ip->dev, addr);
    b = (uint*)bp1->data;

    if((addr = b[bn1 % NINDIRECT]) == 0){
